...
import { backendAddress, getQuillEditorValue, setQuillEditorValue, createStandartQuillEditor, imagesUrlRoot } from "../../services/common";
...
fetch(backendAddress + `/api/advertising/getAll.php`).then((resp) => resp.json()).then((data) => {
    advertisings = data;
...
function addEventListenersToRow(row: HTMLElement) {
    ...
    createStandartQuillEditor(rowElems.descriptionChangeElem);
}
    ...
    return `<div class="table__row" data-is-block="${ad.is_block ? "true" : "false"}" data-order="${ad.order}">
        <div class="table__content-cells-container">
            ...
            <div class="table__cell _description">
                <div>Описание</div>
                <div class="table__cell-change-elem">${ad.description}</div>
            </div>
...
function onCancelBtnClick(event: MouseEvent) {
    ...
    if (rowElems.id.innerText) {
        ...
        setQuillEditorValue(rowElems.descriptionChangeElem, ad.description);
...
async function onSaveBtnClick(event: MouseEvent) {
    ...
    if (rowElems.id.innerText) {
        ...
        const changedAd: Advertising = {
            ...
            description: getQuillEditorValue(rowElems.descriptionChangeElem),
        ...
        await fetch(backendAddress + `/api/advertising/change.php`, {
            method: "put",
        ...
        newAd.id = await (await fetch(backendAddress + `/api/advertising/add.php`, {
            method: "post",
