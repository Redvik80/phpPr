...
const findBtn = comp.querySelector(".find-btn") as HTMLButtonElement;
const findInput = comp.querySelector(".find-input") as HTMLInputElement;
...
fetch(backendAddress + `/api/program/getAll.php`).then((resp) => resp.json()).then((data) => {
    programs = data;
    ...
    findBtn.addEventListener("click", async () => {
        programs = await (await fetch(backendAddress + `/api/program/find.php?find_str=` + encodeURIComponent(findInput.value))).json();
...
function getRowElems(row: HTMLElement) {
    return {
        ...
        linkChangeElem: row.querySelector(".table__cell._link .table__cell-change-elem") as HTMLInputElement,
    ...
    return `<div class="table__row">
        <div class="table__content-cells-container">
            ...
            <div class="table__cell _link">
                ...
                <input class="table__cell-change-elem" name="name" value="${program.link}">
            </div>
        </div>
...
async function onDeleteBtnClick(event: MouseEvent) {
    ...
    const row = getButtonsRow(event.currentTarget as HTMLButtonElement);
    const rowElems = getRowElems(row);
    await fetch(backendAddress + "/api/program/delete.php?id=" + rowElems.id.innerText, { method: "delete" });
...
async function onSaveBtnClick(event: MouseEvent) {
    ...
    if (rowElems.id.innerText) {
        const changedProgram = {
            ...
            duration,
            link: rowElems.linkChangeElem.value
        };
        await fetch(backendAddress + `/api/program/change.php`, {
            method: "put",
        ...
        newProgram.id = await (await fetch(backendAddress + `/api/program/add.php`, {
            method: "post",
